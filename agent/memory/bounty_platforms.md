...
|----------|-------------|------------------|--------|---------------------------|---------------------------|-------|
| **Immunefi** | **Crypto (on-chain)** | USDC/USDT; chain per program (often Ethereum, Arbitrum, Base) | **✅ High** | Account + [wallet verify](https://bugs.immunefi.com/settings/wallets-and-payments) (sign message); wallet at report submit | ⚡ Export (`export_for_manual_submit`) | `/approve bounty` → markdown в `agent/bounty/exports/`; paste на bugs.immunefi.com |
| **HackenProof** | **Crypto (custodial balance → withdraw)** | **USDC on Base** (~95% programs); per-program: ETH, BTC, native tokens | **✅ High** | Account + 2FA + external wallet in profile; min withdraw **100 USDC** | ⚡ Export (`export_for_manual_submit`) | `/approve bounty` → markdown export; submit на `{program}/submit` |
