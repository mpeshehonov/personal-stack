...
class RefreshOpenTest(unittest.TestCase):
    ...
    def test_hh_archived(self) -> None:
        ...
        fake.json.return_value = {"archived": True, "name": "X"}
        with mock.patch("opportunity.refresh_open.httpx.get", return_value=fake):
    ...
    def test_hh_open(self) -> None:
        fake = mock.Mock()
        fake.status_code = 200
        fake.json.return_value = {"archived": False, "name": "FE"}
        with mock.patch("opportunity.refresh_open.httpx.get", return_value=fake):
            out = validate_hh_vacancy_url("https://hh.ru/vacancy/12345")
